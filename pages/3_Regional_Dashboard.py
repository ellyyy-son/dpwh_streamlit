...
st.set_page_config(page_title="Region Specific Dashboard", layout="wide")
...
@st.cache_data
def load_data():
    return pd.read_csv("dataset/final_results.csv")
...
if "region" not in df.columns:
    st.error("The dataset does not contain a 'region' column.")
    st.stop()
...
st.caption(f"{total_projects:,} project(s) in the selected region")
...
if "contractor" in filtered_df.columns:
    contractor_summary = (
        filtered_df.groupby("contractor", dropna=False)
        .agg(
            project_count=("contractId", "count"),
...
if "province" in filtered_df.columns:
    province_summary = (
        filtered_df.groupby("province", dropna=False)
        .agg(
            project_count=("contractId", "count"),
            avg_risk_score=("risk_score", "mean"),
            anomaly_count=("is_anomaly", "sum"),
...
high_risk_cols = [
    col for col in [
        "contractId", "description", "province", "contractor", "status",
...
# ----------------------------
# Full regional dataset
# ----------------------------
st.markdown("### Full Regional Dataset")
